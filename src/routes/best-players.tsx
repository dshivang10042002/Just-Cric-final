  ...
  return (
    <div className={`flex flex-col gap-3 rounded-2xl border bg-card p-4 transition hover:border-primary/30 hover:shadow-md sm:flex-row sm:items-center sm:gap-4 ${isTop3 ? "border-primary/20" : "border-border"}`}>
      ...
      <div className="grid grid-cols-4 gap-2 border-t border-border pt-3 text-center sm:w-auto sm:shrink-0 sm:gap-4 sm:border-l sm:border-t-0 sm:pl-4 sm:pt-0">
        <div>
          <div className={`font-display text-base sm:text-xl tabular-nums ${isTop3 ? rankColors[rank-1] : "text-primary"}`}>{p.score}</div>
        ...
        <div>
          <div className="font-display text-base sm:text-xl tabular-nums text-accent">{p.wkts}</div>
          <div className="text-[9px] uppercase tracking-wider text-muted-foreground">Wkts</div>
        </div>
        <div>
          <div className="font-display text-base sm:text-xl tabular-nums text-foreground">{p.matches}</div>
